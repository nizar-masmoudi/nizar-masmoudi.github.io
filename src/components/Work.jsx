...
const Item = ({ order, src, title }) => (
  <div className='group relative w-3/4 h-20 drop-shadow-md border-[1px] border-white/10 rounded-md overflow-hidden hover:scale-105 hover:drop-shadow-xl duration-200 cursor-pointer'>
    ...
    <div className='absolute inset-0 flex items-center px-4 w-full h-full bg-gradient-to-r from-[#1a1e2a] via-[#1a1e2a] to-[#1a1e2a]/30'>
      ...
      <h2 className='duration-200 text-sm font-medium text-white/70 group-hover:text-white'>{title}</h2>
    </div>
  </div>
);
